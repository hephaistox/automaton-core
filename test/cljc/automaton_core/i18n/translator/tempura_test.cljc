...
(deftest TempuraTranslator-test
  (testing "Is the specified language working"
    (is (= (get-in sut/tempura-missing-text [:en :missing])
           (let [test-translator (sut/make-translator [:fr]
                                                      sut/tempura-missing-text
                                                      dict-stub)]
             (core-translator/translate test-translator
                                        [:en]
                                        :missing
                                        []))))
    (is (= (get-in sut/tempura-missing-text [:fr :missing])
           (let [test-translator (sut/make-translator [:en]
                                                      sut/tempura-missing-text
                                                      dict-stub)]
             (core-translator/translate test-translator
                                        [:fr]
  ...
  (testing "Are the dictionaries found"
    (let [translator (sut/make-translator [:en]
                                          sut/tempura-missing-text
                                          dict-stub)]
      (is (= "bar-en"
             (-> translator

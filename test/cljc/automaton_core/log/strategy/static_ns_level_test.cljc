...
(deftest apply-ns-rule-test
  (let [ns-chooser-stub (sut/make-static-ns-level-strategy sut/ns-rules)
    ...
    (testing "ns matching the scope are accepted"
      (is (map? (sut/apply-ns-rule "automaton-core.adapters.edn-utils" (get-stubbed-rule :ac-edn-utils))))
      (is (map? (sut/apply-ns-rule "automaton-web.duplex.message-handler" (get-stubbed-rule :aw-duplex-messages)))))))
...
(deftest choose-logger-test
  (let [ns-chooser-stub (sut/make-static-ns-level-strategy sut/ns-rules)
    ...
    (testing "Among many matches the first one is returned"
      (is (= [:automaton-core.log.registry/text-based] (apply-strategy* "automaton-core.adapters.edn-utils" :fatal))))
    (testing "Default logger is text-based"
      (is (= [:automaton-core.log.registry/text-based] (apply-strategy* "non-existing-namespace" :fatal))))))

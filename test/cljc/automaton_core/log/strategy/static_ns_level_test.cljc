  ...
  (let [ns-chooser-stub (sut/make-static-ns-level-strategy sut/ns-rules)
        ...
        test-ns (fn [rule-id test-ns]
                  (some-> rule-id
                          ...
                          :re
                          (re-find test-ns)))]
    (testing "Check all used strategies are known strategies in `automaton-core.log.strategy/registry`"
      (is (every? (set (keys log-registry/strategies-registry)) (log-strategy/rule-ids ns-chooser-stub))))
...
(deftest apply-ns-rule-test
  (let [ns-chooser-stub (sut/make-static-ns-level-strategy sut/ns-rules)
    ...
    (testing "ns outside the scope are rejected"
      ...
      (is (nil? (sut/apply-ns-rule "automaton-core" (get-stubbed-rule :rule2)))))
    (testing "ns matching the scope are accepted"
      (is (map? (sut/apply-ns-rule "automaton-core" (get-stubbed-rule :rule1))))
      (is (map? (sut/apply-ns-rule "automaton-core.log" (get-stubbed-rule :rule2))))
      (is (map? (sut/apply-ns-rule "automaton-core.log" (get-stubbed-rule :rule1)))))))
...
(deftest choose-logger-test
  (let [ns-chooser-stub (sut/make-static-ns-level-strategy sut/ns-rules)
    ...
    (testing "One matching is returning the expected match"
      (is (= [:automaton-core.log.registry/print] (apply-strategy* "automaton-core" :fatal))))
    (testing "Among many matches the first one is returned"
      (is (= [:automaton-core.log.registry/print] (apply-strategy* "automaton-core.log" :fatal))))
    (testing "Default logger is text-based and error-tracking"
      (is (= [:automaton-core.log.registry/text-based :automaton-core.log.registry/error-tracking]

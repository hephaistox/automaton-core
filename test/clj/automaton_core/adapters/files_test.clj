...
(deftest create-dir-path
  (let [expected-result (str sut/file-separator "tmp" sut/file-separator "foo" sut/file-separator "bar" sut/file-separator)]
    ...
    (testing "Relative path are working also"
      (is (= (str "tmp" sut/file-separator "foo" sut/file-separator "bar" sut/file-separator) (sut/create-dir-path "tmp" "foo" "bar"))))))
...
(deftest create-file-path
  (let [expected-result (str sut/file-separator "tmp" sut/file-separator "foo" sut/file-separator "bar")]
    ...
    (testing "Relative path are working also"
      (is (= (str "tmp" sut/file-separator "foo" sut/file-separator "bar") (sut/create-file-path "tmp" "foo" "bar"))))))

(deftest change-extension
  (testing "Change the extension"
    (is (= "toto.edn" (sut/change-extension "toto.tmp" ".edn")))
    (is (= "toto.edn.edn" (sut/change-extension "toto.edn.tmp" ".edn")))
    (is (= "toto.edn" (sut/change-extension "toto" ".edn")))))
...
(deftest file-in-same-dir
  ...
  (testing "If the source is file, stored in the same parent"
    (is (= "automaton/automaton-core/foo" (sut/file-in-same-dir "automaton/automaton-core/deps.edn" "foo"))))
  (testing "If the source is file does not exist, store in the same parent"
    (is (= "automaton/automaton-core/foo" (sut/file-in-same-dir "automaton/automaton-core/deps-does-not-exist.edn" "foo")))))
...
(comment
  (keys (sut/create-files-map "foobar" "**{.clj,.cljs,.cljc,.edn}"))
  ;
)

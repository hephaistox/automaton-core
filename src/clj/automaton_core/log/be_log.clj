(ns automaton-core.log.be-log
  ...
  (:require
   [automaton-core.log.be-registry :as log-registry]
   [automaton-core.log.tracking.be-error-tracking :as exs]))

(defn log-init! [{:keys [dsn env]}]
...
(defn- logger-ids-to-logger-fns
  ...
  (reduce (fn [acc logger-id]
            (conj acc (get-in automaton-core.log.be-registry/strategies-registry
                              [logger-id :impl])))
          []
          logger-ids))
...
(defmacro log-exception
  ...
  (when additional-message
    `(log ~logger-ids ~level ~@additional-message))
  (let [ns *ns*
        log-fns (logger-ids-to-logger-fns logger-ids)]

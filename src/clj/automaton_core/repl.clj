...
(defn create-nrepl-files
  ...
  (let [build-configs (files/search-files "" "**build_config.edn")
    ...
    (doseq [nrepl-port nrepl-ports]
      (files/write-file (str repl-port) nrepl-port))))
...
(defn start-repl*
  ...
  (let [repl-port (get-nrepl-port-parameter)]
    ...
    (.addShutdownHook
      (Runtime/getRuntime)
      (Thread. #(do (log/info "SHUTDOWN in progress, stop repl on port `"
                              repl-port
                              "`")
...
(defn start-repl
  ...
  []
  (try
    (start-repl* {:middleware (conj nrepl-mw/cider-middleware

...
(defn delete-files
  ...
  (doseq [file file-list]
    (if (fs/directory? file)
      (do (core-log/debug "Directory " (absolutize file) " is deleted") (fs/delete-tree file))
      (do (core-log/debug "File " (absolutize file) " is deleted") (fs/delete-if-exists file)))))
...
(defn remove-trailing-separator
  ...
  [path]
  (when-not (str/blank? path) (when-let [path (str/trim path)] (str/replace path (re-pattern (str directory-separator "*$")) ""))))
...
(defn rename-recursively
  ...
  * `pattern-replacement` is what to replace, according to the [replace specification](https://clojuredocs.org/clojure.string/replace). e.g. \"foo_$1_bar\" "
  [target-dir file-filter pattern pattern-replacement]
  (core-log/debug "Rename files and directories in " target-dir " , from `" pattern "` to `" pattern-replacement "`")
  ...
  * `starting-regexp` is a regular expression matching the first line of the file "
  ...
  (doseq [[filename file-content] files-map]
    (let [first-line (-> (str/split-lines file-content)
                         first)]
      (when (re-find starting-regexp first-line) (remove-file filename) (core-log/trace "File" filename " has been removed")))))

...
(defn delete-files
  ...
  (doseq [file file-list]
    (if (fs/directory? file)
      (do (log/debug "Directory " (absolutize file) " is deleted") (fs/delete-tree file))
      (do (log/debug "File " (absolutize file) " is deleted") (fs/delete-if-exists file)))))
...
(defn- copy-files-or-dir-validate
  ...
  (when-not (and (sequential? files) (every? #(or (string? %) (= java.net.URL (class %))) files))
    (throw (ex-info "The `files` parameter should be a sequence of string or `java.net.URL`" {:files files}))))
  ...
  * `target-dir` is where files are copied to"
  ...
  (try (fs/create-dirs target-dir)
       (doseq [file files]
         ...
         (if (fs/directory? file)
           (do (log/trace (format "Copy `%s` to `%s`" (absolutize file) (absolutize target-dir)))
               (fs/copy-tree file
                             target-dir
...
(defn is-existing-file? "Check if this the path exist and is not a directory" [path] (and (fs/exists? path) (not (fs/directory? path))))
...
(defn create-dirs
  ...
  (when (is-existing-file? dir)
    (throw (ex-info (format "Can't create a directory `%s` as a file already exists with that name" (absolutize dir)) {:dir dir})))
...
(defn remove-trailing-separator
  ...
  [path]
  (let [path (str/trim path)] (str/replace path (re-pattern (str file-separator "*$")) "")))
...
(defn create-dir-path
  ...
  [& dirs]
  (when-let [file-path (apply create-file-path dirs)] (str file-path file-separator)))
  ...
  * `(files/search-files \"\" \"**{.clj,.cljs,.cljc,.edn}\")` search all clj files in pwd directory"
  ([root pattern options]
   (when-not (directory-exists? root)
     (throw (ex-info (str root " is expected to be a directory")
                     {:root root
                      :pattern pattern
                      :options options})))
   (into []
...
(defn file-prefix "Adds `file:` as a prefix to the string. Usefull when java io resource type of path is needed" [path] (str "file:" path))

(defn for-each "Apply fn-each on each files in a directory" [dir fn-each] (doseq [file (fs/list-dir dir)] (fn-each (str file))))
...
(defn rename-dir
  "Rename a dir"
  [src dst]
  (try (log/trace "Rename directory " src " to " dst)
       (fs/move src (fs/path dst))
       (catch Exception e
...
(defn add-suffix
  ...
  [filename suffix]
  (let [[_ prefix extension] (re-find #"(.*)(\..*)" filename)] (str/join [prefix suffix extension])))
    ...
    (if (empty? files)
      ...
      (let [filename (str (first files))
        ...
        (if (= new-filename filename)
          ...
          (cond (is-existing-file? filename) (do (rename-file filename new-filename) (recur (rest files) true))
                (is-existing-dir? filename) (do (rename-dir filename new-filename) (recur (rest files) true))
  ...
  (let [pattern (remove-trailing-separator (str pattern))
    ...
    (loop [iterations-left 30]
      (if (> iterations-left 0)
        (when (rename-recursively-attempt target-dir file-filter pattern pattern-replacement) (recur (dec iterations-left)))
...
(defn create-files-map
  ...
  * `pattern` is a regular expression as described in [java doc](https://docs.oracle.com/javase/7/docs/api/java/nio/file/FileSystem.html#getPathMatcher(java.lang.String)) "
  ...
  (->> (search-files target-dir pattern)
       (filter is-existing-file?)
       (map (fn [filename] [(str (absolutize filename)) (read-file (absolutize filename))]))
       (into {})))
...
(defn create-temp-dir
  ...
  * none
  ...
  []
  (let [tmp-dir (apply create-dir-path [(conf/read-param [:tests :tmp-dirs] "tmp/tests") (str (uuid/time-based-uuid))])]

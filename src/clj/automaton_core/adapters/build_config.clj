...
(def build-config-repo-link [:publication :link])

(def repo-name [:publication :repo-name])

(def prod-web-link [:run-env :prod-env :web-link])

(def test-web-link [:run-env :test-env :web-link])
...
(defn spit-build-config
  ...
  * `msg` (optional) to add on the top of the file"
  ([app-dir content msg]
   (let [filename (files/create-file-path app-dir build-config-filename)]
     (edn-utils/spit-edn filename content msg)
     filename))

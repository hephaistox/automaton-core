...
(def strategies-registry
  ...
  {::log-registry/print {:description "Simple print"
                         :impl simple-print}
   ::log-registry/text-based {:description "Basic one - sends everything to flatten text (js/console for clojurescript and log4j2 for clojure)"
                              #?@(:cljs
                                  [:impl automaton-core.log.impl.print/log-fn])}
   ...
   ::log-registry/error-tracking {:description "For monitoring exceptions in the application"
                                  #?@(:cljs
                                      [:impl automaton-core.log.tracking.fe-error-tracking/log-fn])}})

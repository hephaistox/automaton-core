...
(defn logger-ids-to-logger-fns
  ...
  (reduce (fn [acc logger-id]
            (conj acc
                  (get-in #?(:cljs automaton-core.log.fe-registry/strategies-registry
                             :clj {})
                          [logger-id :impl])))
          []
          logger-ids))
...
(defmacro log-data
  ...
  (let [ns (str *ns*) log-fns `(automaton-core.log.fe-log/logger-ids-to-logger-fns ~logger-id)] `((apply juxt ~log-fns) ~ns ~level ~data)))

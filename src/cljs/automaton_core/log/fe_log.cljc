...
#?(:cljs (defn log-init! [params]
           (exs/init-error-tracking! params)))

(defn logger-ids-to-logger-fns
  [logger-ids]
  (reduce (fn [acc logger-id]
            (conj acc (get-in #?(:cljs automaton-core.log.fe-registry/strategies-registry
                                 :clj {})
                              [logger-id :impl])))
          []
          logger-ids))
...
(defmacro log
  ...
  (let [ns (str *ns*)
        log-fns `(automaton-core.log.fe-log/logger-ids-to-logger-fns ~logger-id)]
    `((apply juxt ~log-fns) ~ns (str ~level) ~@message)))

(defmacro log-exception
  [logger-id level exception & additional-message]
  (when additional-message
    (log logger-id level additional-message))
  (let [ns (str *ns*)
        log-fns `(automaton-core.log.fe-log/logger-ids-to-logger-fns ~logger-id)]
...
(defmacro log-data
  ...
  (when additional-message
    (log logger-id additional-message))
  (let [ns (str *ns*)
        log-fns `(automaton-core.log.fe-log/logger-ids-to-logger-fns ~logger-id)]

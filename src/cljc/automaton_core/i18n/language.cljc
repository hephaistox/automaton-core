...
(defrecord AutomatonCoreLanguages [languages]
  Languages
    ...
    (dict-languages-ids [_]
      (set (map first
             (filter (fn [[_ language]] (:core-dict? language)) languages)))))
...
(def ^:private core-languages-dict
  ...
  * `desc` description of the language"
  {:fr {:core-dict? true, :ui-text "FR", :desc "Français"},
   :en {:core-dict? true, :ui-text "EN", :desc "English"},
   :pl {:ui-text "PL", :desc "Polski"}})

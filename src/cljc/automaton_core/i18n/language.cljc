...
(defprotocol Languages
  (language [this lang-id] "return the language\nParams:\n  * `lang-id`: language id")
  (languages [this] "return the list of known languages")
  (languages-ids [this] "Set of known ids (set of keywords)")
  (dict-languages-ids [this] "Set of known language ids keywords"))

(defrecord AutomatonCoreLanguages
           [languages]
  Languages
  (language [_ lang-id]
    (get languages lang-id))
  (languages [_]
    languages)
  (languages-ids [_]
    (set
  ...
  (dict-languages-ids [_]
    (set
     (map first (filter (fn [[_ language]]
                          (:core-dict? language))
                        languages)))))
...
(defn make-automaton-core-languages
  ...
  * `selected-languages` is a dictionary which can add data and restrict the use of some languages in subpart of the app
  ...
  [& selected-languages-seq]
  (->AutomatonCoreLanguages (apply merge-languages-map
                                   core-languages-dict
                                   selected-languages-seq)))

...
(defn execute-min-level?
  ...
  * `level-kw` - keyword of the log to test"
  ...
  (when (and level-kw min-level-kw) (apply <= (map (partial get levels-sequence) [min-level-kw level-kw]))))

(defn execute-max-level?
  [max-level-kw level-kw]
  (when (and level-kw max-level-kw) (apply >= (map (partial get levels-sequence) [max-level-kw level-kw]))))
...
(defn execute-level?
  ...
  (cond (and min-level max-level) (and (execute-min-level? min-level level) (execute-max-level? max-level level))

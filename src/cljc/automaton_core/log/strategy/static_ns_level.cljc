...
(def ns-rules
  ...
  * `:logger` - seq of loggers ids to apply, loggers themselves are defined later on, in clj and cljs sides as their implementation depends on the technology most of the time"
  [{:rule-id :rule2 :re #"automaton-core.log" :min-level :debug :logger [::log-registry/print]}
   {:rule-id :rule1 :re #"automaton-core.*" :min-level :info :logger [::log-registry/print]}])
...
(defrecord StaticNsLevelStrategy
           [ns-rules]
  log-strategy/Strategy
  (apply-strategy [_ ns level]
    (let [active-rule (some-> (keep (partial apply-ns-rule ns)
                                    ns-rules)
                              first)]
      (if (some-> active-rule
                  :min-level
...
(defn make-static-ns-level-strategy
  ...
  * `ns-rules` set of rules to feed the chooser"
  ([ns-rules*]
   (->StaticNsLevelStrategy ns-rules*))
  ([]
